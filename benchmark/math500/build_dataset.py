"""Build MATH-500 prompts (500 problems) for Dream-Instruct, 4-shot Minerva style.

The 4 in-context examples are the canonical Minerva-math few-shot prompt used
by lm-eval-harness's `minerva_math` and `math_500` tasks.
"""
...
OUT_DIR = Path(__file__).resolve().parents[2] / "outputs" / "benchmarks" / "math500"
OUT_DIR.mkdir(parents=True, exist_ok=True)
FULL_PATH = OUT_DIR / "prompts_full.jsonl"

# Canonical Minerva 4-shot prompt (lm-eval-harness `minerva_math` task).
FEW_SHOT_PREFIX = r"""Problem:
...
def _build_prompt(problem: str) -> str:
    """4-shot Minerva prefix + the test problem (no .format() — LaTeX braces clash)."""
    return FEW_SHOT_PREFIX + "Problem:\n" + problem + "\n\nSolution:\n"
...
def main() -> None:
    ...
    for i, item in enumerate(ds):
        rows.append({
            ...
            "prompt": _build_prompt(item["problem"]),
        })
