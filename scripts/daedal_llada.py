"""daedal_llada.py — DAEDAL training-free variable-length denoising on LLaDA.

Same algorithm as scripts/daedal_dream.py (verbatim port of
DAEDAL/models/LLaDA_DAEDAL.py::generate()), but routed through LLaDA's forward
(`scripts.llada_backend.forward_logits`) and using LLaDA's mask/EOS ids by
default.

Single-sequence (batch=1). FLOPs counter wraps the entire run.
"""
...
def run_daedal_llada(
    ...
    *,
    ...
    """DAEDAL on LLaDA. Single-sequence (batch=1)."""
    set_seed(seed)
